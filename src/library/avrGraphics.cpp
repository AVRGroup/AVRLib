...
///
#include <avrGraphics.h>   // inclui avrParameters e avrUtil
///
...
static double gWinWidth;
static double gWinHeight;

void argReshapeCallback(int w, int h){
   gWinWidth = w;
   gWinHeight = h;
}
...
void argConvGLcpara(ARParam *param, double gnear, double gfar, double m[16]) {
   int width = param->xsize;
   int height = param->ysize;
...
static void argDispImageTexRectangle(ARUint8 *image, int xwin, int ywin, int mode) {
   ...
   switch(mode) {
      case 0:
         dist_factor = &(gCparam.dist_factor[0]);
         break;
      case 1:
         dist_factor = &(gsCparam.dist_factorL[0]);
         break;
      case 2:
         dist_factor = &(gsCparam.dist_factorR[0]);
         break;
      default:
         return;
   }
   ...
   if(flag[mode][win][list]) {
      ...
      for(j = 1; j <= 20; j++) {
         ...
         for(i = 0; i <= 20; i++) {
            ...
            glTexCoord2d(px, py / size_adjust_factor);
            glVertex3d(xx1, yy1, z);
            glTexCoord2d(px, qy / size_adjust_factor);
            glVertex3d(xx2, yy2, z);
         }
         glEnd();
      }
      glEndList();
   ...
   if(flag[mode][win][list]) {
      ...
      for(j = 1; j <= 20; j++) {
         ...
         for(i = 1; i <= 20; i++) {
            ...
            glBegin(GL_QUADS);
            glTexCoord2d(tsx, tsy);
            glVertex3d(xx1, yy1, z);
            glTexCoord2d(tex, tsy);
            glVertex3d(xx2, yy2, z);
            glTexCoord2d(tex, tey);
            glVertex3d(xx3, yy3, z);
            glTexCoord2d(tsx, tey);
            glVertex3d(xx4, yy4, z);
            glEnd();
         }
      }
      glEndList();
...
void argSetWindowName(const char* name){
   int len = strlen(name);
   windowName = (char*) malloc(len * sizeof(char));
